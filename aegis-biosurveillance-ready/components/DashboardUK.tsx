...
import { 
    kpiDataUK, 
    influenzaTrendDataUK, 
    hfmdTrendDataSingapore, 
    mapDataUK, 
    mockAlertsUK, 
    MOCK_DATA_SUMMARY_UK, 
    threatSpectrumDataUK, 
    UK_REGION_PATHS,
    oneHealthIndexDataUK,
    pestInfestationAlertsSingapore,
    cropDiseaseAlertsSingapore,
} from './constants';
import { calculateOneHealthIndex } from '../services/oneHealthService';

interface DashboardUKProps {
...
const DashboardUK: React.FC<DashboardUKProps> = ({ activeAlert, onEscalate, onAcknowledge, onResolve }) => {
  ...
  const oneHealthIndex = useMemo(() => 
    calculateOneHealthIndex(
      oneHealthIndexDataUK.human,
      oneHealthIndexDataUK.zoonotic,
      oneHealthIndexDataUK.environmental
    ), []);
  ...
  if (showBiosecurityModule) {
    return <BiosecurityDashboard country="United Kingdom" onBack={() => setShowBiosecurityModule(false)} />;
  }
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <OneHealthIndexPanel indexData={oneHealthIndex} country="United Kingdom" />
            <ModuleCard
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <MapChart title="Regional Case Distribution (Singapore)" data={mapDataUK} paths={UK_REGION_PATHS} onStateClick={handleStateSelect} />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <TrendChart title="Influenza Trends" data={influenzaTrendDataUK} color="#F85149" />
              <TrendChart title="HFMD Trends" data={hfmdTrendDataSingapore} color="#58A6FF" />
            </div>
             <AiAnalysisPanel dataSummary={MOCK_DATA_SUMMARY_UK} />
          </div>

          {/* Right column */}
          <div className="space-y-6">
            <AlertsPanel alerts={mockAlertsUK} />
            <ThreatSpectrum threats={threatSpectrumDataUK} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            <ZoonoticSurveillancePanel
              alerts={oneHealthIndexDataUK.zoonotic}
              slaughterhouseAlerts={oneHealthIndexDataUK.slaughterhouses}
            ...
            />
            <EnvironmentalPanel
              alerts={oneHealthIndexDataUK.environmental}
            ...
            />
             <WildlifeSurveillancePanel
              alerts={oneHealthIndexDataUK.wildlife}
            ...
            <DisasterResponsePanel
              alerts={oneHealthIndexDataUK.disasters}
            ...
            />
             <PestCropSurveillancePanel
              pestAlerts={pestInfestationAlertsSingapore}
              cropAlerts={cropDiseaseAlertsSingapore}
              onAlertClick={setPestCropDetail}
              onViewLayer={() => setActiveSurveillanceLayer('pest_crop')}
            />
            <GenomicSurveillancePanel
              alerts={oneHealthIndexDataUK.genomic}
